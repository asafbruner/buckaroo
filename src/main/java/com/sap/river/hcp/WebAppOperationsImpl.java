...
public class WebAppOperationsImpl implements WebAppOperations {
	...
	private static final String ERROR_MSG_LOCATION_CONFLICT = "Confilict in webmvc-config.xml. Resource in location " + LOCATION_ATTR_VALUE  + " is already mapped to a different value.";
...
/**
  	Set the path to the ui5 library.
  	In webmvc-config.xml add the line:
	<mvc:resources location="/, classpath:/META-INF/web-ui5/" mapping="/**"/>
	
	Make sure it is after any other <resources> node.
	If no other <resources> node exists, append to the end of the <beans> node.
*/
 ...
 * 
 * - If the addon's location exists, and mapping is the same - return false - no need to add a node
 * - If the addon's location exists, and mapping is different - break with a proper message. We don't want to override existing nodes.
 * - Else - return true indicating a node should be inserted 
 * 
...
/**
 * Add the new resources node to the right place
 * Assume there are no conflicts
 * If other resources nodes exist - append the new node to the last one.
 * Else - append as the last child of the beans node
 * @param webXMLDoc
 * @param beansElement
 * @param resources
 */
	...
	{
		/**
		 * Insert the new node after the last <resources> node found.
		 * Unfortunately, There is no insertAfter utility, so we:
		 * 
		 * - Clone the last resource node - Insert the clone before the original
		 * - Update the original nodes attributes to "our" values
		 * 
		 */
		if (resources != null && !(resources.isEmpty())) {
			...
			lastResourceElement
					.setAttribute(LOCATION_ATTR, LOCATION_ATTR_VALUE);
			lastResourceElement.setAttribute(MAPPING_ATTR, MAPPING_ATTR_VALUE);
		}
		/**
		 * There are no resources node - append the node as the last child of
		 * <beans>
		 */
		else {
